...
void ple_motion_generator::init_ple_motion_generator(franka::Robot& robot, std::mutex& state_lock,
                                                     franka::RobotState& robot_state)
{
	{
...
(const franka::RobotState& robot_state,
 ...
 const std::function<Eigen::Matrix<double, 7, 1>(double)>& get_joint_position_error)
{
	{
		std::lock_guard state_guard(state_lock_);
		state_ = robot_state;
...
Eigen::Matrix<double, 7, 1> ple_motion_generator::calculate_ple_motion(const franka::RobotState& robot_state,
                                                                       double time)
{
	...
	offset << 0.4 * (0.3 * std::sin(x * 0.6 * M_PI) - 0.3 * std::cos(x * 0.5 * M_PI) + 0.2 * std::sin(x * M_PI) + 0.5 *
			std::cos(x * M_PI)),
		0.4 * (0.2 * std::sin(x * 0.5 * M_PI) + 0.1 * std::cos(x * 0.5 * M_PI) + 0.2 * std::sin(x * M_PI) + 0.3 *
			std::cos(x * M_PI) + 0.2 * std::sin(x * 1.5 * M_PI) + 0.1 * std::cos(x * 1.5 * M_PI)),
		0.4 * (0.2 * std::sin(x * 0.7 * M_PI) + 0.3 * std::cos(x * 0.5 * M_PI) - 0.2 * std::sin(x * M_PI) + 0.2 *
			std::cos(x * M_PI) + 0.1 * std::sin(x * 1.5 * M_PI) + 0.3 * std::cos(x * 1.5 * M_PI) - 0.1 *
			std::sin(x * 2.0 * M_PI) + 0.2 * std::cos(x * 2.0 * M_PI)),
		2. * (0.4 * (0.3 * std::sin(x * 0.8 * M_PI) - 0.2 * std::cos(x * 0.5 * M_PI) + 0.4 * std::sin(x * M_PI) + 0.2 *
			std::cos(x * M_PI) - 0.2 * std::sin(x * 1.5 * M_PI) + 0.1 * std::cos(x * 1.5 * M_PI) - 0.1 * std::cos(
				x * 2.0 * M_PI))),
		0.3 * (-0.1 * std::cos(x * 0.5 * M_PI) - 0.1 * std::sin(x * M_PI) + 0.3 * std::cos(x * M_PI) + 0.2 *
			std::sin(x * 1.5 * M_PI) + 0.1 * std::cos(x * 1.5 * M_PI) + 0.2 * std::sin(x * 2.0 * M_PI) - 0.2 *
			std::cos(x * 2.0 * M_PI) + 0.3 * std::sin(x * 3.0 * M_PI)),
		0.3 * (0.1 * std::sin(x * 0.5 * M_PI) + 0.2 * std::sin(x * M_PI) - 0.2 * std::cos(x * M_PI) - 0.1 *
			std::sin(x * 1.5 * M_PI) + 0.2 * std::cos(x * 1.5 * M_PI) + 0.3 * std::sin(x * 2.0 * M_PI) + 0.1 *
			std::cos(x * 2.0 * M_PI) + 0.2 * std::sin(x * 3.0 * M_PI) * std::sin(x * 3.0 * M_PI)),
		0.3 * (0.3 * std::sin(x * M_PI) + 0.1 * std::cos(x * M_PI) - 0.3 * std::cos(x * 1.5 * M_PI) + 0.1 *
			std::sin(x * 2.0 * M_PI) + 0.2 * std::cos(x * 2.0 * M_PI) + 0.2 * std::sin(x * 3.0 * M_PI));
...
void ple_motion_generator::set_stiffness(const std::array<double, 49>& stiffness)
{
	...
	else
	{
		throw std::runtime_error(
			"(Joint impedance controller) Setting impedance stiffness after initialization is not allowed!");
	}
}
