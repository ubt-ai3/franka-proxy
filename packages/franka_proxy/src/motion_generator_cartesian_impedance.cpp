...
void cartesian_impedance_motion_generator::init_impedance_motion_generator(
 ...
 const std::function<Eigen::Matrix<double, 6, 1>(double)>& get_position_error)
{
	...
	if (online_parameter_calc_)
	{
		...
		for (int i = 0; i < inertia_matrix.rows() - 3; i++)
		{
			...
			{
				auto stability_condition = current_damping - ((current_damping * current_damping * delta_time) / mi) +
					1.0; // ((current_damping * 0.0002 * 0.001) / mi);
	...
	Eigen::Matrix position_error(get_position_error(time_));

	// calculate external force
	Eigen::Matrix<double, 6, 1> f_ext = inertia_matrix * acceleration + damping_matrix_ * velocity + stiffness_matrix_ *
		position_error;
	...
	if (logging_)
	{
		...
		logger_.add_ft_data(stiffness_matrix_(0, 0), stiffness_matrix_(1, 1), stiffness_matrix_(2, 2),
		                    stiffness_matrix_(3, 3), stiffness_matrix_(4, 4), stiffness_matrix_(5, 5));
		logger_.add_ft_data(damping_matrix_(0, 0), damping_matrix_(1, 1), damping_matrix_(2, 2), damping_matrix_(3, 3),
		                    damping_matrix_(4, 4), damping_matrix_(5, 5));
...
Eigen::Matrix<double, 6, 1> cartesian_impedance_motion_generator::calculate_position_error(
	const franka::RobotState& robot_state, double time)
{
	...
	position_error.head(3) << position - position_d;
	// transforming to 6x6 as the position error will be mulitplied with the stiffness matrix
...
double cartesian_impedance_motion_generator::optimizeDamping(double l_di, double u_di, double mi, double bi,
                                                             double x0i_max, double derived_x0i_max)
{
	// di = min(max(...), ...);
...
void cartesian_impedance_motion_generator::set_rotational_stiffness(double rotational_stiffness)
{
	...
	else
	{
		throw std::runtime_error(
			"(Cartesian impedance controller) Setting impedance rotational stiffness after initialization is not allowed!");
	}
}
