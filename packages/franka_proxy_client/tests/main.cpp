...
enum test_mode { none, ple, playback, gripper, ptp, force, vacuum, guiding };
...
{
    constexpr std::chrono::seconds short_delay = std::chrono::seconds(1);
    constexpr std::chrono::seconds medium_delay = std::chrono::seconds(3);
    constexpr std::chrono::seconds long_delay = std::chrono::seconds(10);
    constexpr std::chrono::milliseconds update_sleep_ms = std::chrono::milliseconds(16); // Approx. 60 Hz update rate
    ...
    const std::vector<std::string>& params)
{
    ...
    std::thread status_thread([&stop, &robot]() {
        ...
        while (!stop)
        {
            ...
            if (time_test_constants::print_status_interval > 0 && 
                i % time_test_constants::print_status_interval == 0)
            {
                std::cout << "Status #" << i << ": ";
                print_status(*robot);
            }
    ...
    const std::string& file)
{
    ...
    std::cout << "--- Press Enter to start recording in " << time_test_constants::short_delay.count() << "s (robot lights white) ---\n";
    std::cin.get();
    std::this_thread::sleep_for(time_test_constants::short_delay);
    ...
    std::cout << "--- Press Enter to start reproduction in " << time_test_constants::short_delay.count() << "s (robot lights blue) ---\n";
    std::cin.get();
    std::this_thread::sleep_for(time_test_constants::short_delay);
    ...
    const std::string& file)
{
    ...
    {
        std::cerr << "A Franka proxy exception occurred (not related to pose 4 of 4 being unreachable): " << f.what() << '\n';
        std::cerr << "Type: " << typeid(f).name() << '\n';
        std::cout << "Pose 4 not identified as unreachable. Test failed.\n";
    }
    catch (std::exception& g)
    {
        std::cerr << "A general exception occurred (not related to pose 4 of 4 being unreachable): " << g.what() << '\n';
        std::cerr << "Type: " << typeid(g).name() << '\n';
        std::cout << "Pose 4 not identified as unreachable. Test failed.\n";
    }
    ...
    const double duration)
{
    ...
    // Target position for contact, below starting_pos, slighty above our table
