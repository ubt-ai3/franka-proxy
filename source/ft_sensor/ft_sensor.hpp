...
public:
    ...
    virtual ft_sensor(const Eigen::Affine3f& kms_T_flange, const Eigen::Affine3f& EE_T_kms,
        const std::array<float, 6>& bias , const Eigen::Affine3f& load) = 0;
...
protected:
    std::array<float, 6> bias_; //[fx, fy, fz, tx, ty, tz]
