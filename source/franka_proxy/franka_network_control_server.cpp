	...
	while (!terminate_internal_thread_)
	{
		...
		catch (const std::exception& exc)
		{
			std::cerr << "franka_control_server::task_main(): "
						<< "An exception occurred while processing requests, dropping stream and stopping robot. " << std::endl
...
void franka_control_server::process_request(const std::string& request)
{
	...
	const std::size_t command_string_size = std::strlen(franka_proxy_messages::command_strings[type]);
	...
	auto send_response = [this](unsigned char response)
	{
		...
		std::cout << "franka_control_server::process_request(): Sending response: " << static_cast<int>(msg[0]) << std::endl;
