...
{
	csv_parser::csv_parser(std::string base_path):
		base_path_(base_path)
	{
	}
	
	//Creates the overvbiew.csv file and writes the header
	void csv_parser::create_header(std::array<int, 12>& dim) {
		...
		std::string path (base_path_ + time_string + "/");
		path_ = path;
		CreateDirectoryA(path.c_str(), NULL);

		std::array<std::ofstream, 12> sa_data_files;
		std::array<std::string, 12> filenames;

		for (int d = 0; d < 12; d++) {
			if (dim[d] != 1) continue;

			filenames[d] = base_path_ + time_string + "/sa_overview_" + std::to_string(d) + ".csv";
	...
	//Writes the important information of the last SA iteration in the overview.csv file
	void csv_parser::write_in_overview(std::array<int, 12> dim, int k, int c, double T, double eta, double best_F, double current_F, double new_F, std::array<Eigen::Vector3d, 12>& best_parameters, std::array<Eigen::Vector3d, 12>& current_parameters, std::array<Eigen::Vector3d, 12>& new_parameters) {
		...
		for (int d = 0; d < 12; d++) {
			if (dim[d] != 1) continue;
			std::string filename(path_ + "sa_overview_" + std::to_string(d) + ".csv");

			sa_data_files[d].open(filename, std::ofstream::out | std::ofstream::app);
