...
void playback_test(franka_proxy::franka_remote_interface& robot)
{
	...
	const std::vector<std::array<double, 6>> selection_vectors(record.second.size(), std::array<double, 6>{1, 1, 1, 1, 1, 1});
	...
	for (int i = 0; i < record.first.size(); i++)
		log(csv_log, record.first.at(i), record.second.at(i), i*0.001);
}
